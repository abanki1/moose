...
ADStressDivergenceShell2::computeQpResidual()
{
  ...
  for (_qp_z = 0; _qp_z < _t_weights.size(); ++_qp_z)
  {
    ...
    // std::cout << " AB: _residual1 : before penalty" << residual1 << std::endl; // AB: print out the residual before any penalty
    if (_large_strain)
    {
    ...
    if (_component == 5)
    {
      if (_i == _qp)
      {
        std::cout << "GAMMAz AB:_gamma_z: " << (*_gamma_z[_qp_z])[_qp] << std::endl; // AB: print out shear strain Z
        std::cout << "JWz AB:_ad_JxW[_qp]: " << _ad_JxW[_qp] << std::endl; // AB: print out geometric Jacobian
        std::cout << "COORDz AB:_ad_coord[_qp]: " << _ad_coord[_qp] << std::endl; // AB: print out the quadrature points
        ...
        auto addDiag0 = _penalty * (*_gamma_z[_qp_z])[_qp] / (_ad_JxW[_qp] * _ad_coord[_qp]); //original implementation
        residual1 += addDiag0;
        std::cout << "DIAGz AB: Diagonal addition0 to residual1 : after penalising rot_z" << addDiag0 << std::endl;
    ...
    if (_component == 4)
    {
      if (_i == _qp)
      {
        std::cout << "GAMMAy AB:_gamma_y: " << (*_gamma_y[_qp_z])[_qp] << std::endl; // AB: print out shear strains rot_Y
        std::cout << "JWy: AB:_ad_JxW[_qp] Jacobian: " << _ad_JxW[_qp] << std::endl; // AB: print out geometric JxW
        std::cout << "COORDx: AB: _ad_coord[_qp] coordinate: " << _ad_coord[_qp] << std::endl; // AB: print out the ad coordinates
        // WhatType<decltype(penaltyResidual)>{};
        residual1 += _penalty * (*_gamma_y[_qp_z])[_qp] / (_ad_JxW[_qp] * _ad_coord[_qp]); //original implementation

        auto addDiag = _penalty * (*_gamma_y[_qp_z])[_qp] * (*_gamma_y[_qp_z])[_qp] * (_ad_JxW[_qp]) * (_ad_coord[_qp]);
        auto addDiag0 = _penalty * (*_gamma_y[_qp_z])[_qp] / (_ad_JxW[_qp] * _ad_coord[_qp]); //like original implementation
        residual1 += addDiag0;
        std::cout << "DIAGy AB: Diagonal addition0 to residual1 : after penalising rot_y" << addDiag0 << std::endl;
        // residual1 += _penalty * 4/9 * (_ad_JxW[_qp] * _ad_coord[_qp]); //atempting to fix the penalty //atempting to fix the penalty
      }
    }
    // std::cout << " AB: _residual1 : before penalising rot_x" << residual1 << std::endl; // AB: print out the residual before penalty
    if (_component == 3)
    {
      if (_i == _qp)
      {
        std::cout << "GAMMAx AB: _gamma_x: " << (*_gamma_x[_qp_z])[_qp] << std::endl; // AB: print out shear strains rot_x
        std::cout << "JWx: AB:_ad_JxW[_qp] Jacobian: " << _ad_JxW[_qp] << std::endl; // AB: print out geometric JxW
        std::cout << "COORDx: AB:_ad_coord[_qp] coordinate: " << _ad_coord[_qp] << std::endl; // AB: print out the ad coordinates
        std::cout << "PRODx: AB: _ad_JxW[_qp] * _ad_coord[_qp] coordinate: " << (_ad_JxW[_qp] * _ad_coord[_qp]) << std::endl; // AB: print out the ad coordinates
        std::cout << "DOTx: AB: _gamma_x[_qp_z][_qp]/(_ad_JxW[_qp]): " << (*_gamma_x[_qp_z])[_qp] * (*_gamma_x[_qp_z])[_qp] << std::endl; // AB: print out the ad coordinates
        // WhatType<decltype(penaltyResidual)>{};

        auto addDiag = _penalty * (*_gamma_x[_qp_z])[_qp] * (*_gamma_x[_qp_z])[_qp] * (_ad_JxW[_qp]) * (_ad_coord[_qp]);
        auto addDiag0 = _penalty * (*_gamma_x[_qp_z])[_qp] / (_ad_JxW[_qp] * _ad_coord[_qp]); //original implementation
        residual1 += addDiag0;
        std::cout << "DIAGx AB: Diagonal addition0 to residual1 : after penalising rot_x" << addDiag0 << std::endl;
        ...
        // std::cout << "DDDDD AB: Difference in residual1 : after penalising rot_x" << (1.0e6 * (*_gamma_x[_qp_z])[_qp]) << " do derivs=" << ADReal::do_derivatives << std::endl; // AB: print out the change in residual after penalty
        // std::cout << " AB: _residual1 : after all penalties" << residual1 << std::endl; // AB: print out the residual after penalty
      }
