...
#include "ADComputeIncrementalShellStrain4.h"
#include "MooseMesh.h"
#include "Assembly.h"
...
registerMooseObject("TensorMechanicsApp", ADComputeIncrementalShellStrain4);

InputParameters
ADComputeIncrementalShellStrain4::validParams()
{
    ...
    _sol_old(_nonlinear_sys.solutionOld())
{
  ...
  if (_ndisp != 3 || _nrot != 3)
    mooseError(
        "ADComputeIncrementalShellStrain4: The number of variables supplied in 'displacements' "
        "must be 3 and that in 'rotations' must be 3.");

  if (_mesh.hasSecondOrderElements())
    mooseError(
        "ADComputeIncrementalShellStrain4: Shell element is implemented only for linear elements.");
...
void
ADComputeIncrementalShellStrain4::initQpStatefulProperties()
{
  unsigned int dim = _current_elem->dim();
  if ((dim != 2))
    mooseError(
        "ADComputeIncrementalShellStrain4: Shell element is implemented only for 2D elements");
  if (_current_elem->n_nodes() != 4)
    mooseError("ADComputeIncrementalShellStrain4: Shell element needs to have exactly four nodes.");
...
void
ADComputeIncrementalShellStrain4::computeProperties()
{
  ...
  {
    _global_local_rotation[i] = _global_local_rotation_old[i];
    ...
    for (unsigned int j = 0; j < _t_points.size(); ++j)
    {
      ...
      for (unsigned int temp1 = 0; temp1 < 5; ++temp1)
      {
        ...
        for (unsigned int temp2 = 0; temp2 < 20; ++temp2)
          _strain_vector(temp1) += (*_B[j])[i](temp1, temp2) * _soln_vector_local_5dof(temp2);
      }
...
void
ADComputeIncrementalShellStrain4::computeGMatrix()
{
  // quadrature points in isoparametric space
  _2d_points = _qrule->get_points(); // would be in 2D
...
void
ADComputeIncrementalShellStrain4::computeNodeNormal()
{
  for (unsigned int k = 0; k < _nodes.size(); ++k)
  {
...
void
ADComputeIncrementalShellStrain4::computeBMatrix()
{
...
void
ADComputeIncrementalShellStrain4::computeSolnVector()
{
  _soln_vector.zero();
  _soln_current.zero();
  _soln_vector_local_5dof.zero();
